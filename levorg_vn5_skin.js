...
function Draw(){
	...
	Vsd.DrawNeedle(
		...
		MeterMin, MeterMax, Log.Speed / MaxSpeed, 0xFF0000, 10
	);
	...
	Vsd.DrawNeedle(
		...
		MeterMin, MeterMax, (Log.Tacho / 1000) / MaxTacho, 0xFF0000, 10
	);
	...
	Vsd.DrawArc(
		...
		(MeterCenter - 1) - (Log.Brake / Log.Max.Brake * (MeterCenter - MeterMin - 1)),
	...
	if((360 - MeterCenter - 1) <= Log.Accel / Log.Max.Accel * (MeterCenter - MeterMin - 1)){
		Vsd.DrawArc(
			...
			(Log.Accel / Log.Max.Accel * (MeterCenter - MeterMin - 1)) - (360 - MeterCenter - 1), 0x0000FF
		);
	} else {
		Vsd.DrawArc(
			AccelMeterParam.X, AccelMeterParam.Y, MeterR, MeterR,
			MeterR * 0.93, MeterR * 0.93, MeterCenter,
			(Log.Accel / Log.Max.Accel * (MeterCenter - MeterMin - 1)) + MeterCenter + 1 , 0x0000FF
		);
	}
